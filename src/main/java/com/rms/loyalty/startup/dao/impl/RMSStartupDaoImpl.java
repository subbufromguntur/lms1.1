...
@Service
@Component
public class RMSStartupDaoImpl extends GenericDaoHibernateImpl<Country, Long>
		implements RMStartupDao {
	...
	public List<Country> getStates(String[] countryArray) {
		...
		try {
			return this
					.getListByQuery("from Country where locationType=1 and parent_id="
							+ countryArray[1]);
		} catch (DataAccessException e) {
			logger.info(e.getMessage());
		}
	...
	public List<Country> getCities(String[] citiesArray) {
		...
		try {
			return this
					.getListByQuery("from Country where locationType=2 and parent_id="
							+ citiesArray[1]);
		} catch (DataAccessException e) {
			logger.info(e.getMessage());
		}
