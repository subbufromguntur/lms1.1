...
public class RMSStartupServiceImpl implements RMStartupService {
	...
	private String getStateList(List<Country> statesList) {
		...
		for (Country country : statesList) {
			stateBuffer.append("'<option value='" + country.getCountryName()
	...
	private String getCitiesList(List<Country> citiesList) {
		...
		for (Country cities : citiesList) {
			citiesBuffer.append("'<option value='" + cities.getCountryName()
					+ "," + cities.getId() + "'>" + cities.getCountryName()
					+ "</option>");
		}
		return citiesBuffer.toString();
	}

	public OrganizationDataLoader getClientPreConfiguredData() {
		List<String> parent = this.rmsStartupDao
				.getData("select root_name from master_organization_root");
		List<String> type = this.rmsStartupDao
				.getData("select type_name from master_channel_type");
		List<String> category = this.rmsStartupDao
				.getData("select region_name from master_category_region");
		List<String> glAccountType = this.rmsStartupDao
				.getData("select type_name from master_gl_account_type");

		OrganizationDataLoader organizationDataLoader = new OrganizationDataLoader();
		organizationDataLoader.setCategory(category);
		organizationDataLoader.setGlAccountType(glAccountType);
		organizationDataLoader.setType(type);
		organizationDataLoader.setParentList(parent);

		return organizationDataLoader;
	}
