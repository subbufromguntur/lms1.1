$(function() {
	...
	dropZone($('#dropzone1'), function(file) {
		file1 = file;
		if (file2) {
			resembleControl = resemble(file).compareTo(file2).onComplete(
					onComplete);
		}
	});
	...
	dropZone($('#dropzone2'), function(file) {
		file2 = file;
		if (file1) {
			resembleControl = resemble(file).compareTo(file1).onComplete(
					onComplete);
		}
	});
	...
	function calltest() {
		...
		resembleControl = resemble(image1.src.toString()).compareTo(
				image2.src.toString()).onComplete(onComplete);
