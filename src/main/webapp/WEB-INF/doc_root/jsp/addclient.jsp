...
<a href="#" id="alert"></a><br>
        ...
        <%-- <div class="successMessage" align="center">${message}</div> --%>
			...
			<form:form commandName="clientBean" action="submitClient" method="POST" id="id_client_form">
				<fieldset>
						...
						<div>
							<form:select path="timeZone" name="timeZone" id="id_timeZone" class="search" data-placeholder="Choose a Country" >  
								...
								<c:forEach items="${timeZone}" var="timeZoneObj">
								     <option value="${timeZoneObj}"<c:if test="${timeZoneVal eq timeZoneObj}"> selected="selected"</c:if>>${timeZoneObj}</option>
								</c:forEach>
							</form:select>
						</div>
					</div>
